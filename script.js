...
var scrolled = 100; // A variable to keep track of how far we've scrolled.
...
NextTriangle.addEventListener("click", NextClickHandler, false);

var Asides = document.getElementById('content').getElementsByTagName('aside');


for(i=0;i<Asides.length; i++)
{
	Asides.item(i).addEventListener("click", AsideClick, false);
}
...
function AsideClick(e)
{
	for(i=0;i<Asides.length; i++)
	{
		Asides.item(i).className=Asides.item(i).className.replace( ' pop-out' , '' );
		if(Asides.item(i) == this)
		{
			Asides.item(i).className+=' pop-out';
		}
	}
}
